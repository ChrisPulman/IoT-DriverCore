...
public class CodeGeneratorDeterministicCoverageTests
{
    ...
    /// <summary>Verifies every scalar PLC mapping.</summary>
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), $"TwinCATRx_CodeGen_{Guid.NewGuid()}");
        ...
        try
        {
            ...
            await TUnitAssert.That(generator.CreateDll("public sealed class GeneratedType { }", validPath)).IsTrue();
    ...
    /// <summary>A deterministic node implementation for traversal tests.</summary>
