...
internal sealed class SocketMitsubishiTransport : IMitsubishiTransport
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (_options is null)
        {
            throw new InvalidOperationException(MitsubishiMessages.TransportNotConfigured);
        }
    ...
    private static async ValueTask<byte[]> ReceiveUdpAsync(
        Socket socket,
    ...
    private static async ValueTask<byte[]> ReceiveAsciiTcpAsync(
        Socket socket,
        MitsubishiFrameType frameType,
        CancellationToken cancellationToken)
    {
        var prefixLength = frameType == MitsubishiFrameType.FourE
            ? MitsubishiNumericConstants.Thirty
            : MitsubishiNumericConstants.TwentyTwo;
        ...
        var lengthOffset = frameType == MitsubishiFrameType.FourE
            ? MitsubishiNumericConstants.TwentyTwo
            : MitsubishiNumericConstants.Fourteen;
        var responseDataLength = Convert.ToInt32(
            prefixText[lengthOffset..(lengthOffset + MitsubishiNumericConstants.Four)],
            MitsubishiNumericConstants.Sixteen);
        var remainingAsciiChars = Math.Max(0, responseDataLength - MitsubishiNumericConstants.Two) *
    ...
    private static async ValueTask<byte[]> ReceiveExactlyAsync(
        Socket socket,
        int count,
        CancellationToken cancellationToken)
    {
    ...
    private async ValueTask<byte[]> ReceiveTcpAsync(
        Socket socket,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return await ReceiveExactlyAsync(
                    socket,
                    expectedResponseLength.Value,
                    cancellationToken)
                .ConfigureAwait(false);
        }
        ...
        {
            return await ReceiveAsciiTcpAsync(socket, _options.FrameType, cancellationToken)
                .ConfigureAwait(false);
        }

        var prefixLength = _options.FrameType == MitsubishiFrameType.FourE
            ? MitsubishiNumericConstants.Fifteen
