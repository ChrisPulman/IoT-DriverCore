...
public sealed record MitsubishiTagChange(
    string Name,
    MitsubishiTagDefinition? Previous,
    MitsubishiTagDefinition? Current)
{
    /// <summary>Defines the tolerance for engineering metadata comparisons.</summary>
    private const double MetadataComparisonTolerance = 1e-12;
    ...
    private static MitsubishiSchemaChangeKind ClassifyTagChange(
        MitsubishiTagDefinition? previous,
        MitsubishiTagDefinition? current)
    {
    ...
    private static bool HasDataShapeChange(
        MitsubishiTagDefinition previous,
        MitsubishiTagDefinition current) =>
    ...
    private static bool HasMetadataChange(
        MitsubishiTagDefinition previous,
        MitsubishiTagDefinition current) =>
        !string.Equals(previous.Description, current.Description, StringComparison.Ordinal)
        || Math.Abs(previous.Scale - current.Scale) > MetadataComparisonTolerance
        || Math.Abs(previous.Offset - current.Offset) > MetadataComparisonTolerance
