...
public class NumericConversionViaRxS7Tests
{
    ...
    [Test]
    public async Task ParseBytes_ShouldDecodeWordDWordDInt()
    {
        ...
        await Assert.That(word, Is.EqualTo(0x1234));
        ...
        await Assert.That(dword, Is.EqualTo(0x01020304U));
        ...
        await Assert.That(dint, Is.EqualTo(-1));
    }

    /// <summary>Ensures internal floating parsing roundtrips using S7 big-endian format.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous unit test.</returns>
    [Test]
    public async Task ParseBytes_ShouldDecodeRealAndLReal()
    {
        await Assert.That(DebuggerDisplay, Is.Not.Null);
        using var plc = new RxS7(new(new(CpuType.S7200, "127.0.0.1", 0, 0)));
        ...
        await Assert.That(real, Is.EqualTo(1.0F));
        ...
        await Assert.That(lreal, Is.EqualTo(1.0D));
    }
}
