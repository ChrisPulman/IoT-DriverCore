...
internal sealed class MitsubishiSerialWriteTests
{
    ...
    [Test]
    internal async Task WriteWordsAsyncSerial1CFormat1EncodesExpectedRequestAsync()
    {
        ...
        var result = await client.WriteWordsAsync("D100", [ 0x1234, 0x5678], CancellationToken.None);
        ...
        await Assert.That(Encoding.ASCII.GetString(transport.Requests[0].Payload))
            .IsEqualTo("\u000500FFWW0D01000212345678D5");
    }
    ...
    [Test]
    internal async Task WriteWordsAsyncSerial3CFormat1EncodesExpectedRequestAsync()
    {
        await using var transport = new FakeTransport(
            [BuildAsciiAckResponse(
        ...
        await Assert.That(transport.Requests.Count).IsEqualTo(1);
        await Assert.That(Encoding.ASCII.GetString(transport.Requests[0].Payload))
            .IsEqualTo("\u0005F90000FF0014010000000064D*000212345678AF");
    }
    ...
    [Test]
    internal async Task WriteWordsAsyncSerial4CFormat5EncodesExpectedRequestAsync()
    {
        ...
        await Assert.That(Convert.ToHexString(transport.Requests[0].Payload))
            .IsEqualTo("10021600F80000FFFF03000001140000640000A802003412785610033436");
    }
