...
internal sealed class MitsubishiSerialParityTests
{
    /// <summary>Stores the binary length-field size in bytes.</summary>
    private const int BinaryLengthFieldByteCount = 2;

    /// <summary>Stores the memory word count used by parity tests.</summary>
    private const int MemoryWordCount = 2;
    ...
    [Test]
    internal async Task ReadTypeNameAsyncSerial3CFormat1EncodesExpectedRequestAndParsesResponseAsync()
    {
        await using var transport = new FakeTransport(
            [BuildAsciiPayloadResponse(
        ...
        var result = await client.ReadTypeNameAsync(CancellationToken.None);
        ...
        await Assert.That(Encoding.ASCII.GetString(transport.Requests[0].Payload))
            .IsEqualTo("\u0005F90000FF0001010000AD");
    }

    /// <summary>Verifies serial 4C TypeName response parsing.</summary>
    ...
    internal async Task ReadTypeNameAsyncSerial4CFormat5EncodesExpectedRequestAndParsesResponseAsync()
    {
        ...
        await Assert.That(Convert.ToHexString(transport.Requests[0].Payload))
            .IsEqualTo("10020C00F80000FFFF0300000101000010033037");
    }

    /// <summary>Verifies serial 3C loopback response handling.</summary>
    /// <returns>
    /// The LoopbackAsyncSerial3CFormat1EncodesExpectedRequestAndReturnsEchoedPayload operation result.
    /// </returns>
    [Test]
    internal async Task LoopbackAsyncSerial3CFormat1EncodesExpectedRequestAndReturnsEchoedPayloadAsync()
    {
        await using var transport = new FakeTransport(
            [BuildAsciiPayloadResponse(
                MitsubishiFrameType.ThreeC,
        ...
        await Assert.That(Encoding.ASCII.GetString(transport.Requests[0].Payload))
            .IsEqualTo("\u0005F90000FF00061900000004PINGAD");
    }

    /// <summary>Verifies serial 4C loopback response handling.</summary>
    /// <returns>
    /// The LoopbackAsyncSerial4CFormat5EncodesExpectedRequestAndReturnsEchoedPayload operation result.
    /// </returns>
    [Test]
    internal async Task LoopbackAsyncSerial4CFormat5EncodesExpectedRequestAndReturnsEchoedPayloadAsync()
    {
        ...
        await Assert.That(Encoding.ASCII.GetString(result.Value!)).IsEqualTo("PING");
        await Assert.That(Convert.ToHexString(transport.Requests[0].Payload))
            .IsEqualTo("10021200F80000FFFF03000019060000040050494E4710033543");
    }
    ...
    [Test]
    internal async Task ReadMemoryAsyncSerial3CFormat1EncodesExpectedRequestAndParsesResponseAsync()
    {
        await using var transport = new FakeTransport(
            [BuildAsciiPayloadResponse(
                MitsubishiFrameType.ThreeC,
    ...
    [Test]
    internal async Task ReadMemoryAsyncSerial4CFormat5EncodesExpectedRequestAndParsesResponseAsync()
    {
        await using var transport = new FakeTransport([BuildBinaryDataResponse([0x34, 0x12, 0x78, 0x56])]);
        var options = CreateSerialOptions(
            MitsubishiFrameType.FourC,
            CommunicationDataCode.Binary,
            MitsubishiSerialMessageFormat.Format5);
    ...
    [Test]
    internal async Task WriteMemoryAsyncSerial3CFormat1EncodesExpectedRequestAsync()
    {
        await using var transport = new FakeTransport(
        ...
        var result = await client.WriteMemoryAsync(
        ...
        await Assert.That(result.IsSucceed).IsTrue();
        await Assert.That(Encoding.ASCII.GetString(transport.Requests[0].Payload))
            .IsEqualTo("\u0005F90000FF00161300002000000212345678DE");
    }
    ...
    [Test]
    internal async Task WriteMemoryAsyncSerial4CFormat5EncodesExpectedRequestAsync()
    {
        ...
        await Assert.That(result.IsSucceed).IsTrue();
        await Assert.That(Convert.ToHexString(transport.Requests[0].Payload))
            .IsEqualTo("10021400F80000FFFF03000013160000002002003412785610033643");
    }
    ...
    [Test]
    internal async Task ExecuteRawAsyncSerial3CFormat1EncodesExpectedRequestAndReturnsPayloadAsync()
    {
        await using var transport = new FakeTransport(
            [BuildAsciiPayloadResponse(
                MitsubishiFrameType.ThreeC,
        ...
        await Assert.That(Encoding.ASCII.GetString(transport.Requests[0].Payload))
            .IsEqualTo("\u0005F90000FF001234ABCDBEEF11");
    }
    ...
    [Test]
    internal async Task ExecuteRawAsyncSerial4CFormat5EncodesExpectedRequestAndReturnsPayloadAsync()
    {
        await using var transport = new FakeTransport([BuildBinaryDataResponse([0xCA, 0xFE, 0xBA, 0xBE])]);
        var options = CreateSerialOptions(
            MitsubishiFrameType.FourC,
            CommunicationDataCode.Binary,
            MitsubishiSerialMessageFormat.Format5);
        ...
        await Assert.That(Convert.ToHexString(transport.Requests[0].Payload))
            .IsEqualTo("10021000F80000FFFF0300003412CDABDEADBEEF10034646");
    }
    ...
    private static MitsubishiClientOptions CreateSerialOptions(
        ...
        => new(
            ...
            Timeout: TimeSpan.FromSeconds(SerialTimeoutSeconds),
            CpuType: CpuType.Fx5,
    ...
    private static byte[] BuildAsciiPayloadResponse(
        MitsubishiFrameType frameType,
