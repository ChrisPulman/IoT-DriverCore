...
internal sealed class MitsubishiSerialBitTests
{
    /// <summary>Stores the number of bits read by each test.</summary>
    ...
    [Test]
    internal async Task ReadBitsAsyncSerial1CFormat1EncodesExpectedRequestAndParsesResponseAsync()
    {
        await using var transport = new FakeTransport(
            [BuildAsciiBitReadResponse(MitsubishiFrameType.OneC, MitsubishiSerialMessageFormat.Format1, "1011")]);
        ...
        var result = await client.ReadBitsAsync("M10", BitCount, CancellationToken.None);
    ...
    [Test]
    internal async Task WriteBitsAsyncSerial1CFormat1EncodesExpectedRequestAsync()
    {
        await using var transport = new FakeTransport(
        ...
        var result = await client.WriteBitsAsync("M10", [true, false, true, true], CancellationToken.None);

        await Assert.That(result.IsSucceed).IsTrue();
        await Assert.That(transport.Requests.Count).IsEqualTo(1);
        await Assert.That(Encoding.ASCII.GetString(transport.Requests[0].Payload))
            .IsEqualTo("\u000500FFBW0M0010041011EA");
    }
    ...
    [Test]
    internal async Task ReadBitsAsyncSerial3CFormat1EncodesExpectedRequestAndParsesResponseAsync()
    {
        await using var transport = new FakeTransport(
            [BuildAsciiBitReadResponse(MitsubishiFrameType.ThreeC, MitsubishiSerialMessageFormat.Format1, "1011")]);
        var options = CreateSerialOptions(
        ...
        await Assert.That(transport.Requests.Count).IsEqualTo(1);
        await Assert.That(Encoding.ASCII.GetString(transport.Requests[0].Payload))
            .IsEqualTo("\u0005F90000FF000401000100000AM*04BD");
    }
    ...
    [Test]
    internal async Task WriteBitsAsyncSerial3CFormat1EncodesExpectedRequestAsync()
    {
        await using var transport = new FakeTransport(
            [BuildAsciiAckResponse(
                MitsubishiFrameType.ThreeC,
        ...
        await Assert.That(transport.Requests.Count).IsEqualTo(1);
        await Assert.That(Encoding.ASCII.GetString(transport.Requests[0].Payload))
            .IsEqualTo("\u0005F90000FF001401000100000AM*00041011E1");
    }
    ...
    [Test]
    internal async Task ReadBitsAsyncSerial4CFormat5EncodesExpectedRequestAndParsesResponseAsync()
    {
        await using var transport = new FakeTransport([BuildBinaryBitReadResponse()]);
        var options = CreateSerialOptions(
        ...
        await Assert.That(Convert.ToHexString(transport.Requests[0].Payload))
            .IsEqualTo("10021200F80000FFFF030000010401000A000090040010034146");
    }
    ...
    [Test]
    internal async Task WriteBitsAsyncSerial4CFormat5EncodesExpectedRequestAsync()
    {
        ...
        await Assert.That(transport.Requests.Count).IsEqualTo(1);
        await Assert.That(Convert.ToHexString(transport.Requests[0].Payload))
            .IsEqualTo("10021600F80000FFFF030000011401000A00009004001000101010034633");
    }
    ...
    private static byte[] BuildAsciiBitReadResponse(
        MitsubishiFrameType frameType,
