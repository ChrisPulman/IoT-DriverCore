...
internal sealed class MitsubishiReactiveScanRuntimeTests
{
    ...
    /// <summary>Stores the <c>SetpointTagName</c> test value.</summary>
    private const string SetpointTagName = "Setpoint";

    /// <summary>Stores the reactive polling interval in seconds.</summary>
    private const int ReactivePollingIntervalSeconds = 5;
    ...
    [Test]
    internal async Task ObserveReactiveWordsSharesOneUnderlyingPollForMultipleSubscribersAsync()
    {
        ...
        using var firstSubscription = client
            .ObserveReactiveWords("D100", 1, TimeSpan.FromSeconds(ReactivePollingIntervalSeconds), null)
            .Take(1)
            .Subscribe(first.Add);
        ...
        TestSchedulerDriver.AdvanceBy(scheduler, TimeSpan.FromMilliseconds(1).Ticks);
    ...
    /// <summary>Tests replay to a late subscriber without an extra poll.</summary>
    /// <returns>The ObserveReactiveWordsReplaysLatestValueToLateSubscriberWithoutExtraPoll operation result.</returns>
    [Test]
    internal async Task ObserveReactiveWordsReplaysLatestValueToLateSubscriberWithoutExtraPollAsync()
    {
        var scheduler = new TestScheduler();
        await using var transport = new FakeTransport(
            _ =>
    ...
    [Test]
    internal async Task ObserveReactiveTagGroupUsesSharedReadPlanAndReturnsTypedValuesAsync()
    {
        var scheduler = new TestScheduler();
        await using var transport = new FakeTransport(
            _ =>
        ...
        using var subscription = client
            .ObserveReactiveTagGroup("Line1", TimeSpan.FromSeconds(ReactivePollingIntervalSeconds), null)
            .Take(1)
        ...
        await Assert.That(
                received[0].Value!.GetRequired(new LogicalTagKey<ushort>(RecipeTagName)))
            .IsEqualTo((ushort)0x1234);
        await Assert.That(
                received[0].Value!.GetRequired(new LogicalTagKey<ushort>(SetpointTagName)))
            .IsEqualTo((ushort)0x5678);
        await Assert.That(transport.Requests.Count).IsEqualTo(1);
    ...
    [Test]
    internal async Task ObserveReactiveTagValueUsesTypedTagConversionAsync()
    {
        var scheduler = new TestScheduler();
        await using var transport = new FakeTransport(
            _ =>
        ...
        using var subscription = client
            .ObserveReactiveTag(
                new LogicalTagKey<ushort>(RecipeTagName),
                TimeSpan.FromSeconds(ReactivePollingIntervalSeconds),
                null)
            .Take(1)
            .Subscribe(received.Add);
