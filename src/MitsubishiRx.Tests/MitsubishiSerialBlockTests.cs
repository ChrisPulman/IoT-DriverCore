...
internal sealed class MitsubishiSerialBlockTests
{
    ...
    /// <summary>Tests the 3C format 1 block-read request and its raw payload.</summary>
    /// <returns>The ReadBlocksAsyncSerial3CFormat1EncodesExpectedRequestAndReturnsRawPayload operation
    /// result.</returns>
    [Test]
    internal async Task ReadBlocksAsyncSerial3CFormat1EncodesExpectedRequestAndReturnsRawPayloadAsync()
    {
        await using var transport = new FakeTransport(
            [BuildAsciiBlockReadResponse(
        ...
        await Assert.That(Encoding.ASCII.GetString(transport.Requests[0].Payload))
            .IsEqualTo("\u0005F90000FF000406000000010001000064D*000200000AM*0003FC");
    }
    ...
    [Test]
    internal async Task WriteBlocksAsyncSerial3CFormat1EncodesExpectedRequestAsync()
    {
        await using var transport = new FakeTransport(
        ...
        await Assert.That(transport.Requests.Count).IsEqualTo(1);
        await Assert.That(Encoding.ASCII.GetString(transport.Requests[0].Payload))
            .IsEqualTo("\u0005F90000FF001406000000010001000064D*00021122334400000AM*0003100010B3");
    }

    /// <summary>Tests the 4C format 5 block-read request and its raw payload.</summary>
    /// <returns>The ReadBlocksAsyncSerial4CFormat5EncodesExpectedRequestAndReturnsRawPayload operation
    /// result.</returns>
    [Test]
    internal async Task ReadBlocksAsyncSerial4CFormat5EncodesExpectedRequestAndReturnsRawPayloadAsync()
    {
        await using var transport = new FakeTransport([BuildBinaryBlockReadResponse()]);
        var options = CreateSerialOptions(
        ...
        await Assert.That(Convert.ToHexString(transport.Requests[0].Payload))
            .IsEqualTo("10021C00F80000FFFF0300000604000001000100640000A802000A000090030010034343");
    }
    ...
    [Test]
    internal async Task WriteBlocksAsyncSerial4CFormat5EncodesExpectedRequestAsync()
    {
        ...
        await Assert.That(transport.Requests.Count).IsEqualTo(1);
        await Assert.That(Convert.ToHexString(transport.Requests[0].Payload))
            .IsEqualTo("10022300F80000FFFF0300000614000001000100640000A80200221144330A000090030010001010034144");
    }
    ...
    private static byte[] BuildAsciiBlockReadResponse(
        MitsubishiFrameType frameType,
