...
{
    /// <summary>The default Modbus endpoint port.</summary>
    private const int DefaultModbusPort = 502;

    /// <summary>The number of values displayed for each Modbus data collection.</summary>
    private const int UiCollectionLength = 50;

    /// <summary>The server data polling interval in milliseconds.</summary>
    private const double DataObservationIntervalMilliseconds = 100;
    ...
    private void SetupCommands()
    {
        ...
        _ = RemoveClientCommand.ThrownExceptions.Subscribe(
            ex => StatusMessage = $"Error removing client: {ex.Message}");
    ...
    /// <returns>A task that completes after initialization is attempted.</returns>
        ...
        try
        {
            ...
            foreach (var clientConfig in enabledClients)
            {
                try
                {
                    ...
                    {
                        "TCP" => _server.AddTcpClient(
                            clientConfig.Name,
                            clientConfig.Address,
                            clientConfig.Port,
                            clientConfig.SlaveId),
                        "UDP" => _server.AddUdpClient(
                            clientConfig.Name,
                            clientConfig.Address,
                            clientConfig.Port,
                            clientConfig.SlaveId),
                        _ => throw new NotSupportedException(
                            $"Connection type {clientConfig.ConnectionType} not supported"),
                    };
    ...
    private void ClearData()
    {
        ...
        _server.LoadSimulationData(
            new ushort[UiCollectionLength],
            new ushort[UiCollectionLength],
            new bool[UiCollectionLength],
            new bool[UiCollectionLength]);
    }
    ...
    private async Task AddClientAsync()
    {
        try
        {
            ...
            NewClientPort = DefaultModbusPort;
    ...
    private void UpdateData((ushort[] HoldingRegisters, ushort[] InputRegisters, bool[] Coils, bool[] Inputs) data)
    {
        ...
        UpdateRegisterCollection(HoldingRegisters, CopyFirst(data.HoldingRegisters, UiCollectionLength));

        // Update input registers
        UpdateRegisterCollection(InputRegisters, CopyFirst(data.InputRegisters, UiCollectionLength));

        // Update coils
        UpdateCoilCollection(Coils, CopyFirst(data.Coils, UiCollectionLength));

        // Update inputs
        UpdateCoilCollection(Inputs, CopyFirst(data.Inputs, UiCollectionLength));
    }
}
