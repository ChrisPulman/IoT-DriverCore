...
[DebuggerDisplay("IsOpen = {_port.IsOpen}, Pending = {_pending.Count}")]
public sealed class SerialPortRxMessageHandler : IDisposable
{
    /// <summary>The default request timeout when the port has no positive read timeout.</summary>
    private const int DefaultReadTimeoutMilliseconds = 3000;

    /// <summary>The minimum request timeout.</summary>
    private const int MinimumReadTimeoutMilliseconds = 100;

    /// <summary>The delay between polling iterations.</summary>
    private const int PollingDelayMilliseconds = 100;

    /// <summary>The error message used when the port is closed.</summary>
    ...
    private readonly ISerialPortRx _port;
    ...
    public SerialPortRxMessageHandler(ISerialPortRx port, params string[]? errorLine)
    {
    ...
    /// A <see cref="Task" /> representing the asynchronous operation.
    /// </returns>
    public async Task WithPollingStoppedAsync(Func<Task>? action)
    {
        if (action is null)
        {
            return;
        ...
        _pollingTask = Task.Run(
            async () =>
            {
                while (!token.IsCancellationRequested)
                {
                    ...
                    await Task.Delay(PollingDelayMilliseconds, token).ConfigureAwait(false);
                }
            },
            token);
    }
    ...
    private bool IsEchoLine(string trimmed)
    {
        ...
        if (head.Command?.Trim() is not { Length: > 0 } sent)
        {
            return false;
        }
        ...
        if (ResponsePrefix is not { Length: > 0 } prefix)
        {
            return false;
        }
    ...
    private void CompleteNextPending(string trimmed)
    {
        ...
        try
        {
            ...
            if (prefix is { Length: > 0 } && toApply.StartsWith(prefix, StringComparison.Ordinal))
            {
