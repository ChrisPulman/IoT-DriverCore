...
public class ModbusRxSerialRtuSlaveFixture : NetworkTestBase
{
    ...
    public async Task ModbusRxSerialRtuSlave_BonusCharacter_VerifyTimeoutAsync()
    {
        ...
        Skip.IfNot(
            !IsRunningInCI,
            "Serial port tests require physical hardware not available in CI");

#if SERIAL
        var masterPort = ModbusRxMasterFixture.CreateAndOpenSerialPort(
            ModbusRxMasterFixture.DefaultMasterSerialPortName);
        var slavePort = ModbusRxMasterFixture.CreateAndOpenSerialPort(
            ModbusRxMasterFixture.DefaultSlaveSerialPortName);
