...
{
    /// <summary>The Modbus slave address used by simulation tests.</summary>
    private const byte SlaveAddress = 1;

    /// <summary>The first valid register index.</summary>
    private const int FirstRegisterIndex = 1;

    /// <summary>The second valid register index.</summary>
    private const int SecondRegisterIndex = 2;

    /// <summary>The third valid register index.</summary>
    private const int ThirdRegisterIndex = 3;

    /// <summary>The interval between simulation updates, in milliseconds.</summary>
    private const int SimulationUpdateIntervalMilliseconds = 50;

    /// <summary>The initial delay that allows a simulation to produce data, in milliseconds.</summary>
    private const int InitialSimulationDelayMilliseconds = 200;

    /// <summary>The delay used when comparing generated patterns, in milliseconds.</summary>
    private const int PatternComparisonDelayMilliseconds = 100;

    /// <summary>The delay that allows server simulation values to change, in milliseconds.</summary>
    private const int SimulationChangeDelayMilliseconds = 600;

    /// <summary>The number of registers read when comparing simulated values.</summary>
    private const int RegisterReadCount = 10;

    /// <summary>The expected value stored in the third counting-pattern register.</summary>
    private const ushort ThirdRegisterExpectedValue = 2;

    /// <summary>The loopback address used by the simulated server connection.</summary>
    ...
    public void SimulationDataProvider_SineWaveGeneration_ShouldBeRealistic()
    {
        ...
        // Act
        var sineData = SimulationDataProvider.GenerateSineWave(length, amplitude, 1.0, 0.0);
    ...
    [TUnit.Core.Test]
    public async Task SimulationDataProvider_ContinuousSimulation_ShouldUpdateDataAsync()
    {
        // Arrange
        ...
        {
            dataStore.HoldingRegisters[FirstRegisterIndex],
            dataStore.HoldingRegisters[SecondRegisterIndex],
            dataStore.HoldingRegisters[ThirdRegisterIndex],
        };

        // Act
        provider.Start(
            dataStore,
            TimeSpan.FromMilliseconds(SimulationUpdateIntervalMilliseconds),
            SimulationType.Random);
        await Task.Delay(InitialSimulationDelayMilliseconds); // Let it run for several update cycles
        provider.Stop();
    ...
    /// <returns>A <see cref="Task"/> representing the asynchronous unit test.</returns>
    [TUnit.Core.Test]
    public async Task SimulationDataProvider_DifferentTypes_ShouldProduceDifferentPatternsAsync()
    {
        // Arrange
        using var provider = new SimulationDataProvider();
        ...
        await Task.Delay(PatternComparisonDelayMilliseconds);
        provider.Stop();

        provider.Start(
            dataStore2,
            TimeSpan.FromMilliseconds(SimulationUpdateIntervalMilliseconds),
            SimulationType.SineWave);
        await Task.Delay(PatternComparisonDelayMilliseconds);
        provider.Stop();

        // Assert - Should produce different patterns
        var values1 = CopyFirst(dataStore1.HoldingRegisters, RegisterReadCount);
        var values2 = CopyFirst(dataStore2.HoldingRegisters, RegisterReadCount);
    ...
    /// <returns>A <see cref="Task"/> representing the asynchronous unit test.</returns>
    [TUnit.Core.Test]
    public async Task ModbusServer_WithSimulation_ShouldServeRealtimeDataAsync()
    {
        // Arrange
        ...
        _ = server.StartTcpServer(tcpPort, SlaveAddress);
        ...
        var reading1 = await master.ReadHoldingRegistersAsync(SlaveAddress, 0, RegisterReadCount);
        await Task.Delay(SimulationChangeDelayMilliseconds); // Wait for simulation to update
        var reading2 = await master.ReadHoldingRegistersAsync(SlaveAddress, 0, RegisterReadCount);
    ...
    public void SimulationDataProvider_BooleanPatterns_ShouldBeCorrect()
    {
        ...
        {
            Assert.Equal(i % SecondRegisterIndex == 0, alternating[i]);
        }

        // Act & Assert - Random pattern (should have variation)
        var random = provider.GenerateBooleanPattern(PatternComparisonDelayMilliseconds, BooleanPattern.Random);
    ...
    public void SimulationDataProvider_MixedDataTypes_ShouldLoadCorrectly()
    {
        ...
        Assert.Equal(0, dataStore.HoldingRegisters[FirstRegisterIndex]); // First real value
        Assert.Equal(1, dataStore.HoldingRegisters[SecondRegisterIndex]); // Second real value
        Assert.Equal(
            ThirdRegisterExpectedValue,
            dataStore.HoldingRegisters[ThirdRegisterIndex]); // Third real value

        // Check that input registers match
        Assert.Equal(
            dataStore.HoldingRegisters[FirstRegisterIndex],
            dataStore.InputRegisters[FirstRegisterIndex]);
        Assert.Equal(
            dataStore.HoldingRegisters[SecondRegisterIndex],
            dataStore.InputRegisters[SecondRegisterIndex]);

        // Check coils (should reflect register values) - use 1-based indexing
        Assert.False(dataStore.CoilDiscretes[FirstRegisterIndex]); // 0 % 2 == 0 -> false
        Assert.True(dataStore.CoilDiscretes[SecondRegisterIndex]); // 1 % 2 == 1 -> true
        Assert.False(dataStore.CoilDiscretes[ThirdRegisterIndex]); // 2 % 2 == 0 -> false

        // Check inputs (inverted coils) - use 1-based indexing
        Assert.True(dataStore.InputDiscretes[FirstRegisterIndex]); // !false
        Assert.False(dataStore.InputDiscretes[SecondRegisterIndex]); // !true
        Assert.True(dataStore.InputDiscretes[ThirdRegisterIndex]); // !false
    }
    ...
    /// <param name="values">The values to inspect.</param>
    /// <returns>
    /// <c>true</c> when the sequence contains both <c>true</c> and <c>false</c>;
    /// otherwise, <c>false</c>.
    /// </returns>
