...
public static class DInt
{
    ...
    /// <summary>Converts a 64-bit signed value to the S7 32-bit signed representation.</summary>
    ...
    /// <returns>A 32-bit signed integer representing the converted value. For values greater than <see
    /// cref="int.MaxValue"/>, a
    ...
    /// <param name="bytes">The byte array containing the bytes to convert to an integer. The array must
    ...
    /// signed integer. The first four
    /// bytes are used for the conversion.</param>
    /// <returns>A 32-bit signed integer represented by the first four bytes of the span, interpreted as
    /// big-endian.</returns>
    ...
    /// <param name="destination">The span of bytes that will receive the big-endian representation of the
    /// value. Must be at least 4 bytes in
    /// length.</param>
    ...
    /// <param name="destination">A span of bytes that receives the binary data. Must be at least four times
    /// the length of <paramref
    /// name="values"/>.</param>
    /// <exception cref="ArgumentException">Thrown if <paramref name="destination"/> is not large enough to
    /// contain the binary representations of all
    /// values.</exception>
    ...
    /// <returns>A byte array containing the binary representation of the input integer array. The length of
    /// the returned array
