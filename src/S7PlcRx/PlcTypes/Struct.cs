...
public static class Struct
{
    ...
    /// <exception cref="ArgumentException">Thrown if a string field in the struct does not have the required
    ...
    /// <param name="structType">The type of the structure to deserialize the byte array into. Must be a type
    /// with a parameterless constructor
    /// and supported field types.</param>
    /// <param name="bytes">The byte array containing the serialized data for the structure. The length must
    /// match the expected size of the
    /// structure.</param>
    /// <returns>An object representing the deserialized structure, or null if the byte array is null or does
    /// not match the
    /// expected size.</returns>
    /// <exception cref="ArgumentException">Thrown if an instance of the specified type cannot be created, or
    /// if a string field is missing the required
    ...
    /// conversion.</remarks>
    /// <param name="structValue">The structure object to convert to a byte array. Must not be null. The
    /// object's fields must be of supported
    /// types.</param>
    /// <returns>A byte array containing the serialized representation of the structure. Returns an empty
    /// array if <paramref
    /// name="structValue"/> is null.</returns>
    /// <exception cref="ArgumentException">Thrown if a field value cannot be converted to its corresponding
    /// type, or if a string field is missing the
    ...
    private static double GetIncreasedNumberOfBytes(double numBytes, FieldInfo info) =>
        info.FieldType == typeof(System.TimeSpan)
            ? IncrementedToEven(numBytes) + sizeof(int)
            : Type.GetTypeCode(info.FieldType) switch
            {
                ...
                TypeCode.Int32 or TypeCode.UInt32 or TypeCode.Single => IncrementedToEven(numBytes) + sizeof(int),
                TypeCode.Double => IncrementedToEven(numBytes) + sizeof(double),
                TypeCode.String => IncrementedToEven(numBytes) + GetRequiredStringAttribute(info).ReservedLengthInBytes,
    ...
    private static void SetFieldValueFromBytes(
        ...
        ref double numBytes)
    {
        var typeCode = Type.GetTypeCode(info.FieldType);
    ...
    /// <param name="bytes">The source bytes.</param>
    /// <param name="numBytes">The current byte count.</param>
    private static void SetOtherFieldValueFromBytes(
        FieldInfo info,
        object structValue,
        byte[] bytes,
        ref double numBytes)
    ...
    private static void SetInt32FieldFromBytes(FieldInfo info, object structValue, byte[] bytes, ref double numBytes)
    {
        ...
        var sourceUInt = DWord.FromBytes(
            ...
            bytes[(int)numBytes]);
        info.SetValue(structValue, ConversionExtensions.ConvertToInt(sourceUInt));
        numBytes += sizeof(int);
    }
    ...
    /// <param name="bytePos">The current byte position.</param>
    /// <param name="numBytes">The current byte count.</param>
    private static void SetBooleanFieldBytes(
        FieldInfo info,
        object structValue,
        byte[] bytes,
        ref int bytePos,
    ...
    /// <param name="bytePos">The current byte position.</param>
    /// <param name="numBytes">The current byte count.</param>
    private static void SetByteFieldBytes(
        FieldInfo info,
        object structValue,
        byte[] bytes,
        ref int bytePos,
    ...
    private static TValue GetValueOrThrow<TValue>(FieldInfo fieldInfo, object structValue)
        where TValue : struct => (fieldInfo.GetValue(structValue) as TValue?) ??
            throw new ArgumentException(
                $"Failed to convert value of field {fieldInfo.Name} of {structValue} to type {typeof(TValue)}");
        ...
        foreach (var attribute in fieldInfo.GetCustomAttributes<S7StringAttribute>())
        {
            if (result is not null)
            {
                throw new InvalidOperationException(
                    $"Multiple {nameof(S7StringAttribute)} attributes were found on {fieldInfo.Name}.");
            }
