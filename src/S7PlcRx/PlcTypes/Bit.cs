...
public static class Bit
{
    ...
    /// <param name="byteIndex">The zero-based index of the byte within <paramref name="bytes"/> to examine.
    /// Must be less than the length of
    /// <paramref name="bytes"/>.</param>
    /// <param name="bitIndex">The zero-based index of the bit within the selected byte to check. Must be in
    /// the range 0 to 7.</param>
    /// <returns>true if the specified bit is set; otherwise, false.</returns>
    /// <exception cref="ArgumentOutOfRangeException">Thrown if <paramref name="byteIndex"/> is greater than
    /// or equal to the length of <paramref name="bytes"/>, or if
    ...
    /// <summary>Converts bytes to a BitArray.</summary>
    /// <param name="bytes">The byte array to convert. Each byte is interpreted in order, with the least
    /// significant bit first in each byte.</param>
    /// <returns>A BitArray containing the bits from the input byte array. If the input array is null or
    /// empty, returns an empty
    /// BitArray.</returns>
    public static BitArray ToBitArray(byte[] bytes) => ToBitArray(bytes.AsSpan(), bytes?.Length * BitsPerByte);

    /// <summary>Creates a BitArray from a read-only byte span.</summary>
    /// <param name="bytes">A read-only span of bytes whose bits will be copied into the resulting BitArray.
    /// Each byte is interpreted in
    /// little-endian order, with the least significant bit first.</param>
    /// <returns>A BitArray containing the bits from the input span. The length of the BitArray will be equal
    /// to the total number
    ...
    /// <summary>Converts bytes to a BitArray with an optional bit limit.</summary>
    /// <param name="bytes">The array of bytes to convert to a BitArray. Cannot be null.</param>
    /// <param name="length">The optional number of bits to include in the BitArray. If specified, only the
    /// first length bits are included;
    ...
    /// <returns>A BitArray containing the bits from the specified byte array, limited to the specified length
    /// if provided.</returns>
    ...
    /// <param name="bytes">The span of bytes to convert to a BitArray. The span must not be empty and must
    /// contain enough data to represent
    /// the requested number of bits.</param>
    /// <param name="length">The number of bits to include in the resulting BitArray. Must not be null and
    /// must not exceed the total number
    ...
    /// <exception cref="ArgumentException">Thrown if bytes is empty or if length is greater than the total
    /// number of bits available in bytes.</exception>
    ...
    /// <param name="byteIndex">The zero-based index of the byte within <paramref name="bytes"/> whose bit
    /// will be modified. Must be less than
    /// the length of <paramref name="bytes"/>.</param>
    /// <param name="bitIndex">The zero-based index of the bit to modify within the specified byte. Must be in
    /// the range 0 to 7.</param>
    /// <param name="value">The value to assign to the specified bit. If <see langword="true"/>, the bit is
    /// set; if <see langword="false"/>,
    /// the bit is cleared.</param>
    /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="byteIndex"/> is greater than
    /// or equal to the length of <paramref name="bytes"/>, or
    ...
    /// <param name="bitPositions">A span of tuples specifying the positions of bits to extract. Each tuple
    /// contains the zero-based index of the
    /// byte and the zero-based index of the bit within that byte.</param>
    /// <returns>An array of Boolean values indicating the state of each requested bit. Each element is <see
    /// langword="true"/> if
    /// the corresponding bit is set; otherwise, <see langword="false"/>.</returns>
    public static bool[] GetBits(
        ReadOnlySpan<byte> bytes,
        ReadOnlySpan<(int ByteIndex, int BitIndex)> bitPositions)
    {
    ...
    /// behavior.</remarks>
    /// <param name="bytes">The span of bytes in which bits will be set or cleared. Each update modifies a bit
    /// within this span.</param>
    /// <param name="bitUpdates">A read-only span of tuples specifying the byte index, bit index, and value to
    /// set for each bit. Each tuple
    /// indicates which bit to update and whether to set it to <see langword="true"/> or <see
    /// langword="false"/>.</param>
    public static void SetBits(
        Span<byte> bytes,
        ReadOnlySpan<(int ByteIndex, int BitIndex, bool Value)> bitUpdates)
    {
