...
/// <summary>Converts between S7 Timer bytes and .NET numeric types.</summary>
...
public static class Timer
{
    ...
    /// <param name="bytes">The byte array containing the bytes to convert. Must represent a valid double
    /// value in the expected byte order.</param>
    ...
    /// <returns>A double-precision floating-point number represented by the eight bytes starting at the
    ...
    /// <exception cref="ArgumentException">Thrown if the span does not contain at least 2 bytes starting from
    /// the specified position.</exception>
    public static double FromByteArray(ReadOnlySpan<byte> bytes, int start)
    {
        ...
        var txt = ConversionExtensions.ValToBinString(value);
        var wert = ConversionExtensions.BinStringToInt32(
    ...
    /// <param name="bytes">A read-only span of bytes containing the binary representation of the double
    /// value. The span must contain at
    ...
    /// <param name="bytes">The read-only span of bytes to convert. The length must be a multiple of 2, with
    /// each pair of bytes representing
    ...
    /// <summary>Writes an unsigned 16-bit value as big-endian bytes.</summary>
    ...
    /// <param name="destination">The span of bytes that will receive the two-byte representation of the
    /// value. Must be at least 2 bytes in
    /// length.</param>
