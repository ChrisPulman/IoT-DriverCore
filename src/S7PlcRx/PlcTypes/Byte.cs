...
{
    /// <summary>Converts the specified byte value to a single-element byte array.</summary>
    ...
    /// <summary>Writes the specified byte value into the first position of the provided destination span.</summary>
    /// <param name="value">The byte value to write to the destination span.</param>
    /// <param name="destination">The span of bytes that will receive the value. Must have a length of at
    /// least 1.</param>
    /// <exception cref="ArgumentException">Thrown when <paramref name="destination"/> has a length less than
    /// 1.</exception>
    ...
    /// <summary>Copies the contents of the specified read-only byte span to the destination span.</summary>
    /// <param name="values">The read-only span containing the bytes to copy.</param>
    /// <param name="destination">The span that receives the copied bytes. Must be at least as large as
    /// <paramref name="values"/>.</param>
    /// <exception cref="ArgumentException">Thrown when <paramref name="destination"/> is smaller than
    ...
    /// <summary>Creates a byte value from the specified byte array.</summary>
    ...
    /// <summary>Returns the first byte from the specified read-only span.</summary>
    /// <param name="bytes">A read-only span of bytes from which to retrieve the first byte. Must contain at
    /// least one byte.</param>
    /// <returns>The first byte in the <paramref name="bytes"/> span.</returns>
    /// <exception cref="ArgumentException">Thrown when <paramref name="bytes"/> does not contain at least one
    /// byte.</exception>
