...
public static class String
{
    ...
    /// <returns>A string representation of the decoded UTF-8 text. Returns an empty string if the array is
    /// empty.</returns>
    ...
    /// <returns>A string representation of the specified range of bytes, or an empty string if the range
    ...
    /// <summary>Decodes ASCII bytes from a read-only span to a string.</summary>
    /// <param name="bytes">A read-only span containing the bytes to decode as an ASCII string.</param>
    /// <returns>A string that represents the decoded ASCII characters. Returns an empty string if <paramref
    /// name="bytes"/> is
    /// empty.</returns>
    ...
    /// <returns>A byte array containing the ASCII-encoded bytes of the input string, or an empty array if the
    /// input is null or
    /// empty.</returns>
    ...
    /// <summary>Encodes a string as ASCII bytes into a destination span.</summary>
    ...
    /// <param name="destination">The span to which the encoded ASCII bytes are written. Must be large enough
    /// to hold the encoded bytes.</param>
    /// <returns>The number of bytes written to the destination span. Returns 0 if the input string is null or
    /// empty.</returns>
    /// <exception cref="ArgumentException">Thrown if the destination span is not large enough to contain the
    ...
    /// <summary>Attempts to encode a string as ASCII bytes into a buffer.</summary>
    ...
    /// <param name="bytesWritten">When this method returns, contains the number of bytes written to the
    /// destination buffer. Set to 0 if the input
    /// string is null or empty.</param>
    /// <returns>true if the string was successfully encoded and written to the destination buffer; otherwise,
    /// false.</returns>
