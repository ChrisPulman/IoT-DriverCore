...
public static class S7WString
{
    ...
    /// <summary>Decodes an S7 WString byte array to a .NET string.</summary>
    ...
    /// <param name="bytes">The byte array containing the S7 WString data, including the 4-byte header. Must
    /// not be null and must have a
    ...
    /// <exception cref="PlcException">Thrown if the input array is null, too short, contains malformed S7
    /// WString data, or if decoding fails.</exception>
    public static string FromByteArray(byte[] bytes)
    {
        if (bytes is null || bytes.Length < HeaderLengthInBytes)
        {
        ...
        catch (Exception e)
        {
            throw new PlcException(
                ErrorCode.ReadData,
                $"Failed to parse {VarType.S7WString} from data. Following fields were read: " +
                $"size: '{size}', actual length: '{length}', total number of bytes (including header): " +
                $"'{bytes.Length}'.",
                e);
        }
    }

    /// <summary>Encodes a string as big-endian Unicode with a length prefix.</summary>
    ...
    /// <returns>A byte array containing a 4-byte header followed by the big-endian Unicode bytes of the
    /// string, padded to the
    ...
    public static byte[] ToByteArray(string? value, int reservedLength)
    {
        ...
        var stringLength = Encoding.BigEndianUnicode.GetBytes(
            value,
            0,
            value.Length,
            buffer,
            HeaderLengthInBytes) / BytesPerCharacter;
        if (stringLength > reservedLength)
        {
            throw new ArgumentException(
                $"The provided string length ({stringLength} is larger than the specified reserved length " +
                $"({reservedLength}).");
        }
