...
public static class S7String
{
    ...
    /// <summary>Gets or sets the encoding used for S7String serialization.</summary>
    /// <value>
    /// The string encoding.
    /// </value>
    ...
    /// or
    /// Failed to parse {VarType.S7String} from data. Following fields were read: size: '{size}', actual
    /// length: '{length}', total number of bytes (including header): '{bytes.Length}'.
    /// </exception>
    ...
    public static string FromSpan(ReadOnlySpan<byte> bytes)
    {
        ...
        if (bytes.Length < HeaderLengthInBytes + length)
        {
            throw new PlcException(
                ErrorCode.ReadData,
                $"Insufficient data for S7 String. Expected {HeaderLengthInBytes + length} bytes, got {bytes.Length}");
        }
        ...
        catch (Exception e)
        {
            throw new PlcException(
                ErrorCode.ReadData,
                $"Failed to parse {VarType.S7String} from data. Following fields were read: " +
    ...
    /// <returns>A <see cref="T:byte[]" /> containing the string header and string value with a maximum length
    ...
    public static int ToSpan(string? value, int reservedLength, Span<byte> destination)
    {
        ...
        if (bytes.Length > reservedLength)
        {
            throw new ArgumentException(
                $"The provided string length ({bytes.Length}) is larger than the specified reserved length " +
                $"({reservedLength}).");
        }
    ...
    public static bool TryToSpan(string? value, int reservedLength, Span<byte> destination, out int bytesWritten)
    {
        ...
        if (value is null || reservedLength > MaximumReservedLength ||
            destination.Length < HeaderLengthInBytes + reservedLength)
        {
            return false;
        }
