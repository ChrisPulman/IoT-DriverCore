...
/// <summary>Converts between S7 Counter bytes and unsigned 16-bit values.</summary>
...
public static class Counter
{
    ...
    /// <summary>Reads an unsigned 16-bit value from bytes at an index.</summary>
    /// <param name="bytes">The byte array containing the data to convert. Cannot be null.</param>
    /// <param name="start">The zero-based index in the array at which to begin reading bytes. Must be within
    ...
    /// </summary>
    /// <param name="bytes">A read-only span of bytes containing at least two elements. The first two bytes
    /// are used to construct the 16-bit
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="bytes"/> contains fewer than two
    /// bytes.</exception>
    ...
    /// <summary>Creates an unsigned 16-bit value from low and high bytes.</summary>
    /// <remarks>The returned value is constructed by placing <paramref name="highValue"/> in the high-order
    /// position and <paramref name="lowValue"/> in the low-order position. This is commonly used when converting from
    ...
    public static ushort FromBytes(byte lowValue, byte highValue) => (ushort)((highValue << 8) | lowValue);
    ...
    /// <returns>An array of <see cref="ushort"/> values representing the converted data from the input byte
    /// array.</returns>
    public static ushort[] ToArray(byte[] bytes) => ToArray(bytes.AsSpan());

    /// <summary>Converts a read-only span of bytes to an array of 16-bit unsigned integers.</summary>
    ...
    /// ignored.</remarks>
    /// <param name="bytes">A read-only span of bytes containing the data to convert. The length must be a
    /// multiple of 2.</param>
    /// <returns>An array of <see cref="ushort"/> values parsed from the specified byte span. The array length
    /// is half the length
    ...
    /// <summary>Writes an unsigned 16-bit value in big-endian order.</summary>
    ...
    /// bytes.</param>
    /// <exception cref="ArgumentException">Thrown when <paramref name="destination"/> is less than 2 bytes in
    /// length.</exception>
    ...
    /// <param name="destination">The span of bytes to copy the encoded values into. Must be at least twice
    /// the length of <paramref
    /// name="values"/>.</param>
    /// <exception cref="ArgumentException">Thrown when <paramref name="destination"/> is not large enough to
    /// hold the encoded bytes.</exception>
    ...
    /// <summary>Converts the specified 16-bit unsigned integer to a byte array in little-endian order.</summary>
    ...
    /// <param name="value">An array of <see cref="ushort"/> values to convert. Cannot be <see
    /// langword="null"/>.</param>
    /// <returns>A byte array representing the binary data of the input <see cref="ushort"/> array.</returns>
    /// <exception cref="ArgumentNullException">Thrown if <paramref name="value"/> is <see
