...
public static class LReal
{
    ...
    /// <param name="bytes">The byte array containing the binary representation of a double-precision
    /// floating-point value. Must be at least
    ...
    /// used.</remarks>
    /// <param name="bytes">A read-only span of bytes containing at least 8 bytes representing a
    /// double-precision floating-point value in
    ...
    /// <summary>Converts an S7 32-bit signed value to a double.</summary>
    ...
    /// <summary>Converts an unsigned 32-bit value to a double.</summary>
    /// <param name="value">The 32-bit unsigned integer value to convert.</param>
    /// <returns>A double-precision floating-point number that represents the specified 32-bit unsigned
    /// integer.</returns>
    ...
    /// <param name="bytes">The byte array to convert. The length must be a multiple of the size of a double
    /// (8 bytes).</param>
    ...
    /// <param name="bytes">A read-only span of bytes representing the binary data to convert. The length must
    /// be a multiple of 8, as each
    /// double value is represented by 8 bytes.</param>
    /// <returns>An array of double values parsed from the specified byte span. The length of the array is
    ...
    /// <summary>Writes a double to a span in big-endian order.</summary>
    ...
    /// <param name="destination">The span of bytes that will receive the 8-byte big-endian representation of
    /// the value. Must be at least 8 bytes
    /// in length.</param>
    ...
    /// <param name="destination">The span of bytes that receives the byte representations of the values. Must
    /// be at least values.Length Ã— 8 bytes
    /// in length.</param>
    /// <exception cref="ArgumentException">Thrown when the length of destination is less than values.Length
    /// Ã— 8 bytes.</exception>
    ...
    /// <summary>Converts a double to its 8-byte representation.</summary>
    ...
    /// <returns>A byte array containing the binary representation of the input double array. The array will
    /// be empty if the
