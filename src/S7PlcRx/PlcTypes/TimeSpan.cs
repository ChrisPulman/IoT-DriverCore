...
{
    /// <summary>The size, in bytes, of the type.</summary>
    public static readonly int TypeLengthInBytes = sizeof(int);

    /// <summary>Gets the minimum S7 time value.</summary>
    ...
    /// <param name="bytes">A byte array containing the binary representation of a TimeSpan. The array must be
    ...
    /// <summary>Creates a TimeSpan from S7 milliseconds in a byte span.</summary>
    /// <param name="bytes">A read-only span of bytes containing the 32-bit integer value, in little-endian
    /// format, representing the number
    /// of milliseconds for the TimeSpan. Must be at least 4 bytes in length.</param>
    /// <returns>A TimeSpan that represents the time interval specified by the 32-bit integer value, in
    /// milliseconds, contained
    ...
    /// <summary>Converts a byte array to an array of <see cref="System.TimeSpan"/> values.</summary>
    ...
    /// <param name="bytes">The byte array containing the binary representation of one or more <see
    /// cref="System.TimeSpan"/> values. The
    ...
    /// <param name="bytes">A read-only span of bytes representing one or more 64-bit signed integer values,
    /// each corresponding to a
    /// duration in milliseconds.</param>
    /// <returns>An array of TimeSpan values created from the input bytes. Each element represents a duration
    /// corresponding to
    /// one 64-bit integer value in the input.</returns>
    /// <exception cref="ArgumentOutOfRangeException">Thrown when the length of bytes is not a multiple of the
    /// size of a 64-bit signed integer.</exception>
    public static System.TimeSpan[] ToArray(ReadOnlySpan<byte> bytes)
    {
        if (bytes.Length % TypeLengthInBytes != 0)
        {
            throw new ArgumentOutOfRangeException(
                nameof(bytes),
                bytes.Length,
                $"Parsing an array of TimeSpan requires a multiple of {TypeLengthInBytes} bytes of input data, input data is '{bytes.Length}' long.");
        }
    ...
    /// <summary>Converts a TimeSpan to its S7 byte representation.</summary>
    /// <param name="timeSpan">The <see cref="System.TimeSpan"/> value to convert to a byte array.</param>
    /// <returns>A byte array containing the binary representation of the specified <see
    /// cref="System.TimeSpan"/> value.</returns>
    ...
    /// <summary>Converts an array of <see cref="System.TimeSpan"/> values to a byte array representation.</summary>
    /// <param name="timeSpans">An array of <see cref="System.TimeSpan"/> values to convert. Cannot be null.</param>
    /// <returns>A byte array containing the serialized representation of the input <see
    /// cref="System.TimeSpan"/> values. The
    ...
    /// <summary>Writes a TimeSpan in its S7 time representation.</summary>
    ...
    /// <param name="destination">The span of bytes to which the encoded S7 time value will be written. Must
    /// be at least 4 bytes in length.</param>
    /// <exception cref="ArgumentException">Thrown if <paramref name="destination"/> is less than 4 bytes in
    /// length.</exception>
    /// <exception cref="ArgumentOutOfRangeException">Thrown if <paramref name="timeSpan"/> is less than the
    /// minimum or greater than the maximum value supported by
    ...
    /// <summary>Writes TimeSpan values in their S7 byte representation.</summary>
    /// <param name="timeSpans">The read-only span containing the <see cref="System.TimeSpan"/> values to
    /// convert.</param>
    /// <param name="destination">The span of bytes that receives the binary representation of the <paramref
    /// name="timeSpans"/> values. Must be
    /// large enough to hold all converted values.</param>
    /// <exception cref="ArgumentException">Thrown when <paramref name="destination"/> is not large enough to
    /// contain the binary representation of all
