...
public static class Int
{
    ...
    /// <summary>Converts a byte array to a 16-bit signed integer.</summary>
    /// <param name="bytes">The byte array containing the bytes to convert. Must contain at least two bytes
    /// starting at index zero.</param>
    ...
    /// <summary>Reads a signed 16-bit value from bytes at an index.</summary>
    ...
    /// <returns>A 16-bit signed integer represented by the two bytes starting at the specified index in the
    /// array.</returns>
    ...
    /// encoding.</remarks>
    /// <param name="bytes">A read-only span of bytes containing at least two bytes. The first two bytes are
    /// used to construct the 16-bit
    /// signed integer.</param>
    /// <returns>A 16-bit signed integer represented by the first two bytes of the span, interpreted as
    /// big-endian.</returns>
    /// <exception cref="ArgumentException">Thrown when the length of <paramref name="bytes"/> is less than
    /// 2.</exception>
    ...
    /// <summary>Creates a 16-bit signed integer from two bytes, using the specified low and high byte values.</summary>
    /// <remarks>The bytes are combined in little-endian order, with lowValue as the least significant byte and
    /// highValue as the most significant byte.</remarks>
    /// <param name="lowValue">The low-order byte of the 16-bit value.</param>
    /// <param name="highValue">The high-order byte of the 16-bit value.</param>
    /// <returns>A 16-bit signed integer formed by combining the specified low and high bytes.</returns>
    public static short FromBytes(byte lowValue, byte highValue) => (short)((highValue << 8) | lowValue);

    /// <summary>Converts the specified byte array to an array of 16-bit signed integers.</summary>
    ...
    /// <returns>An array of 16-bit signed integers representing the converted values from the input byte
    /// array.</returns>
    public static short[] ToArray(byte[] bytes) => ToArray(bytes.AsSpan());

    /// <summary>Converts a read-only span of bytes to an array of 16-bit signed integers.</summary>
    ...
    /// <summary>Writes a signed 16-bit value in big-endian order.</summary>
    ...
    /// <param name="destination">The span of bytes that receives the big-endian representation of the value.
    /// Must be at least 2 bytes in length.</param>
    ...
    /// <summary>Writes signed 16-bit values to bytes in little-endian order.</summary>
    /// <param name="values">The source span containing the 16-bit signed integer values to write.</param>
    /// <param name="destination">The destination span where the bytes will be written. Must be at least twice
    /// the length of <paramref
    /// name="values"/>.</param>
    /// <exception cref="ArgumentException">Thrown if <paramref name="destination"/> is not large enough to
    /// contain the converted bytes.</exception>
    ...
    /// <summary>Converts the specified 16-bit signed integer to a byte array.</summary>
    ...
    /// <summary>Converts an array of 16-bit signed integers to a byte array.</summary>
