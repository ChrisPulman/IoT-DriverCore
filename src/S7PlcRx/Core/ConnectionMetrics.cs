...
internal class ConnectionMetrics
{
    ...
    internal TimeSpan AverageSendTime
    {
        get
        {
            lock (_lock)
    ...
    internal TimeSpan AverageReceiveTime
    {
        get
        {
            lock (_lock)
    ...
    internal double ErrorRate => OperationCount > 0 ? (double)ErrorCount / OperationCount : 0;
    ...
    internal void RecordSend(TimeSpan duration, long bytes)
    {
    ...
    internal void RecordReceive(TimeSpan duration, long bytes)
    {
    ...
    /// <summary>Records an error for monitoring and diagnostics.</summary>
    ...
    /// call.</remarks>
    /// <returns>A new <see cref="ConnectionMetrics"/> instance containing the current values of all tracked metrics.
    /// The
