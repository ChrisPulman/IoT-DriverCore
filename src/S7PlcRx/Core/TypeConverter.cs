...
/// require a user-supplied converter function to handle the specific conversion logic for the value type. This class
...
internal static class TypeConverter
{
    ...
    /// <summary>Converts value-type values to bytes with the specified converter.</summary>
    ...
    /// A function that converts each element of type T to its byte array representation. Cannot be null.
    /// </param>
    /// <returns>
    /// A byte array containing the concatenated byte representations of all elements in the input array. Returns an
    /// empty array if the input array is empty.
    /// </returns>
    internal static byte[] ToByteArray<T>(T[] value, Func<T, byte[]> converter)
        where T : struct
    {
        if (value.Length == 0)
        {
    ...
    /// <summary>Converts bytes to value-type values with the specified converter.</summary>
    ...
    /// <param name="converter">
    /// A function that converts a byte array segment to an instance of type T. Cannot be null.
    /// </param>
    /// <returns>
    /// An array of type T containing the converted elements. The array will be empty if the input byte array is empty.
    /// </returns>
    internal static T[] ToArray<T>(byte[] bytes, Func<byte[], T> converter)
        where T : struct => ToArray(bytes.AsSpan(), converter);

    /// <summary>
    /// Converts a read-only span of bytes into an array of value types by applying a converter function to each
    /// segment representing a single value.
    /// </summary>
    ...
    /// <param name="bytes">
    /// The read-only span of bytes to be converted. The length must be a multiple of the size of type T.
    /// </param>
    /// <param name="converter">
    /// A function that converts a byte array representing a single value of type T into an instance of T.
    /// </param>
    /// <returns>An array of type T containing the converted values from the input byte span.</returns>
    internal static T[] ToArray<T>(ReadOnlySpan<byte> bytes, Func<byte[], T> converter)
        where T : struct
    {
        var typeSize = Marshal.SizeOf<T>();
