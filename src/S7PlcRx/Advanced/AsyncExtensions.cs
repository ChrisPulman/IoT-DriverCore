...
public static class AsyncExtensions
{
    ...
    public static ValueTask<T?> ReadValueAsync<T>(
        IRxS7 plc,
        T typeValue,
        string? variable,
    ...
    /// <param name="typeValue">A value used to infer the PLC value type.</param>
        ...
        /// <returns>A <see cref="ValueTask{TResult}"/> that resolves to tag values keyed by tag name.</returns>
    public static ValueTask<Dictionary<string, T?>> ReadValuesAsync<T>(
        IRxS7 plc,
        T typeValue,
    ...
    public static ValueTask WriteValuesAsync<T>(
        IRxS7 plc,
    ...
    public static ReactiveUI.Primitives.Async.IObservableAsync<T?> ObserveValue<T>(
        IRxS7 plc,
        T typeValue,
        string? variable)
        {
            ...
            return ToAsyncObservable(plc.Observe(new LogicalTagKey<T>(variableName)));
        }
    ...
    public static ReactiveUI.Primitives.Async.IObservableAsync<Dictionary<string, T?>> ObserveValues<T>(
        IRxS7 plc,
        T typeValue,
        params string[] variables)
        {
            ...
            return ToAsyncObservable(AdvancedExtensions.ObserveBatch(plc, typeValue, variables));
        }
#endif
    ...
    private static ReactiveUI.Primitives.Async.IObservableAsync<T> ToAsyncObservable<T>(IObservable<T> source) =>
        ReactiveUI.Primitives.Async.SignalAsync.Create<T>((observer, cancellationToken) =>
        {
            ...
            return ValueTask.FromResult(
                ReactiveUI.Primitives.Async.DisposableAsyncExtensions.ToDisposableAsync(subscription));
        });
    ...
    private static Task<T?>[] CreatePendingReads<T>(
        IRxS7 plc,
        IReadOnlyList<string> variables,
        CancellationToken cancellationToken)
    {
        ...
        for (var i = 0; i < variables.Count; i++)
        {
            pendingReads[i] = cancellationToken.CanBeCanceled
                ? plc.ReadAsync(new LogicalTagKey<T>(variables[i]), cancellationToken)
                : plc.ReadAsync(new LogicalTagKey<T>(variables[i]));
        }
    ...
    /// <returns>The resulting value.</returns>
    private static async Task<Dictionary<string, T?>> ReadValuesCoreAsync<T>(
        IReadOnlyList<string> variables,
        Task<T?>[] pendingReads)
    {
    ...
    private static bool TryGetCurrentValue<T>(IRxS7 plc, string variable, out T? value)
    {
        ...
        if (tag is not null &&
            (typeof(T) == typeof(object) || (tag.Type == typeof(T) && tag.Value?.GetType() == typeof(T))))
        {
            value = (T?)tag.Value;
            return true;
        }
    ...
    /// <returns>The resulting value.</returns>
    private static bool TryGetCurrentValues<T>(
        IRxS7 plc,
        IReadOnlyList<string> variables,
