...
internal sealed class PerformanceCounter
{
    ...
    /// <summary>Initializes a new instance of the <see cref="PerformanceCounter"/> class.</summary>
    /// <param name="timeProvider">The time provider; defaults to <see cref="TimeProvider.System"/>.</param>
    internal PerformanceCounter(TimeProvider? timeProvider = null)
    {
    ...
    internal long TotalOperations { get; private set; }

    /// <summary>Gets the total number of errors.</summary>
    internal long TotalErrors { get; private set; }
    ...
    /// thread-safe.</remarks>
    /// <param name="responseTime">The duration taken to complete the operation. Represents the response time to be
    /// recorded.</param>
    internal void RecordOperation(TimeSpan responseTime)
    {
        lock (_lock)
        {
            TotalOperations++;
    ...
    /// <returns>The average operations per second as a double. Returns 0 if no time has elapsed since tracking
    /// started.</returns>
    internal double GetOperationsPerSecond()
    {
        lock (_lock)
        {
            var elapsed = _timeProvider.GetUtcNow().UtcDateTime - _startTime;
    ...
    /// <returns>The average response time, in milliseconds, of all recorded responses. Returns 0 if no response times
    /// have been
    /// recorded.</returns>
    internal double GetAverageResponseTime()
    {
        lock (_lock)
        {
    ...
    /// <returns>A double value representing the error rate. Returns 0 if there are no recorded operations or
    /// errors.</returns>
    internal double GetErrorRate()
    {
        lock (_lock)
        {
