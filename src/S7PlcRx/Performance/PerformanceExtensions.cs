...
/// <summary>
/// Provides compositional methods for IRxS7 PLC instances to enable advanced performance monitoring, optimized read and
...
public static class PerformanceExtensions
{
    ...
    private static readonly ConcurrentDictionary<string, PerformanceCounter> PerformanceCounters = new();

    /// <summary>Stores the c on ne ct io nm et ri c s used by this instance.</summary>
    private static readonly ConcurrentDictionary<string, SimpleConnectionMetrics> ConnectionMetrics = new();
    ...
    /// made.</remarks>
    /// <param name="plc">The PLC to monitor.</param>
    /// <param name="monitoringInterval">The interval at which performance metrics are sampled. If null, defaults to 30
    /// seconds.</param>
    /// <returns>An observable sequence of <see cref="PerformanceMetrics"/> objects containing performance data for the
    /// PLC,
    ...
    public static IObservable<PerformanceMetrics> MonitorPerformance(IRxS7 plc, TimeSpan? monitoringInterval) =>
        MonitorPerformance(plc, monitoringInterval, TimeProvider.System);
    ...
    /// <param name="plc">The PLC from which to read the tags.</param>
    /// <param name="tagNames">A collection of tag names to read from the PLC. Cannot be null. If empty, an empty
    /// dictionary is returned.</param>
    /// <param name="typeMarker">A value that allows the compiler to infer the tag value type.</param>
    /// <param name="optimizationConfig">An optional configuration that controls read optimization behavior, such as
    /// enabling parallel reads and setting
    /// inter-group delays. If null, default optimization settings are used.</param>
    /// <returns>A dictionary mapping each requested tag name to its read value. If a tag cannot be read, an exception
    /// is thrown
    /// and the dictionary may be incomplete.</returns>
    /// <exception cref="ArgumentNullException">Thrown if <paramref name="plc"/> or <paramref name="tagNames"/> is
    /// null.</exception>
    /// <exception cref="InvalidOperationException">Thrown if reading a tag fails due to a PLC communication error or
    /// invalid tag name.</exception>
    public static async Task<Dictionary<string, T?>> ReadOptimizedAsync<T>(
            IRxS7 plc,
    ...
    /// <param name="plc">The PLC to which values are written.</param>
    /// <param name="values">A dictionary containing tag names and their corresponding values to write. Cannot be
    /// null.</param>
    /// <param name="optimizationConfig">An optional configuration object that controls optimization behavior, such as
    /// enabling parallel writes, write
    /// verification, and inter-group delays. If null, default optimization settings are used.</param>
    /// <returns>A task that represents the asynchronous operation. The result contains details about successful and
    /// failed
    /// writes, timing information, and any overall errors encountered during the operation.</returns>
    /// <exception cref="ArgumentNullException">Thrown if <paramref name="plc"/> or <paramref name="values"/> is
    /// null.</exception>
    /// <exception cref="InvalidOperationException">Thrown if write verification is enabled and a written value does not
    /// match the value read back from the PLC.</exception>
    public static Task<Optimization.WriteOptimizationResult> WriteOptimizedAsync<T>(
            IRxS7 plc,
            Dictionary<string, T> values,
            Optimization.WriteOptimizationConfig? optimizationConfig) =>
        WriteOptimizedAsync(plc, values, optimizationConfig, TimeProvider.System);
    ...
    /// <exception cref="InvalidOperationException">Thrown if write verification is enabled and a written value does not
    /// match the value read back from the PLC.</exception>
    public static async Task<Optimization.WriteOptimizationResult> WriteOptimizedAsync<T>(
            IRxS7 plc,
            Dictionary<string, T> values,
            Optimization.WriteOptimizationConfig? optimizationConfig,
            TimeProvider timeProvider)
    {
        ...
        try
        {
            ...
            foreach (var group in groupedWrites)
            {
                ...
                // Add delay between groups if configured
                groupIndex++;
                await DelayBetweenGroupsAsync(config.InterGroupDelayMs, groupIndex, groupedWrites.Count);
            }
        }
        catch (Exception ex)
        {
        ...
        result.EndTime = timeProvider.GetUtcNow().UtcDateTime;
        return result;
    }

    /// <summary>Runs latency, throughput, and reliability benchmarks.</summary>
    ...
    /// <param name="plc">The PLC to benchmark.</param>
    /// <param name="benchmarkConfig">An optional configuration object specifying benchmark parameters. If null, default
    /// settings are used.</param>
    /// <returns>A task that represents the asynchronous operation. The result contains detailed benchmark metrics and
    /// scores for
    /// the PLC.</returns>
    /// <exception cref="ArgumentNullException">Thrown if <paramref name="plc"/> is null.</exception>
    public static Task<BenchmarkResult> RunBenchmarkAsync(IRxS7 plc, BenchmarkConfig? benchmarkConfig) =>
        RunBenchmarkAsync(plc, benchmarkConfig, TimeProvider.System);
    ...
    public static async Task<BenchmarkResult> RunBenchmarkAsync(IRxS7 plc, BenchmarkConfig? benchmarkConfig, TimeProvider timeProvider)
    {
        ...
        try
        {
            ...
            await BenchmarkLatencyAsync(plc, result, config);

            // Test throughput
            await BenchmarkThroughputAsync(plc, result, config);

            // Test reliability
            await BenchmarkReliabilityAsync(plc, result, config);
        ...
        result.EndTime = timeProvider.GetUtcNow();
        return result;
    }
    ...
    /// threads.</remarks>
    /// <param name="plc">The PLC for which to retrieve performance statistics.</param>
    /// <returns>A PerformanceStatistics object containing metrics such as total operations, error rate, average
    /// response time,
    /// connection uptime, and reconnection count for the specified PLC.</returns>
    /// <exception cref="ArgumentNullException">Thrown if <paramref name="plc"/> is null.</exception>
    public static PerformanceStatistics GetPerformanceStatistics(IRxS7 plc) =>
        GetPerformanceStatistics(plc, TimeProvider.System);
    ...
    private static async Task<KeyValuePair<string, T?>> ReadOptimizedTagAsync<T>(
        IRxS7 plc,
        ...
        string tagName)
    {
        try
        {
            var value = await plc.ReadAsync(new LogicalTagKey<T>(tagName));
        ...
        KeyValuePair<string, T> kvp)
    {
        try
        {
            ...
            if (config.VerifyWrites)
            {
                ...
                var readBack = await plc.ReadAsync(new LogicalTagKey<T>(kvp.Key));
    ...
    private static Task DelayBetweenGroupsAsync(int delayMs, int groupIndex, int groupCount) =>
        delayMs > 0 && groupIndex < groupCount ? Task.Delay(delayMs) : Task.CompletedTask;

    /// <summary>Groups tag names by their associated data block.</summary>
    ...
    /// <returns>A dictionary where each key is a data block identifier and the corresponding value is a list of tag
    /// names
    ...
    /// <summary>Groups tag-value pairs by their associated data block.</summary>
    ...
    /// <param name="values">A dictionary containing tag names as keys and their corresponding values to be
    /// grouped.</param>
    /// <param name="plc">The PLC instance used to resolve tag metadata.</param>
    /// <returns>A dictionary where each key is a data block name and the value is a dictionary of tag-value pairs
    /// belonging to
    ...
    /// <param name="tagName">The name of the tag for which to retrieve the data block address. If the tag is not found,
    /// the tag name is used
    /// as the address.</param>
    /// <param name="plc">The PLC instance used to locate the tag.</param>
    /// <returns>A string containing the data block address corresponding to the specified tag. If the tag is not found,
    /// returns
    ...
    {
        var res = TagOperations.GetTag(plc, tagName);
    ...
    /// <returns>A string containing the extracted data block identifier if the address is valid and formatted
    /// correctly;
    ...
    /// <param name="result">The object in which the measured latency statistics and any errors encountered during the
    /// benchmark are
    /// recorded.</param>
    /// <param name="config">The configuration settings that specify how the latency benchmark is performed, including
    /// the number of test
    /// iterations.</param>
    /// <returns>A task that represents the asynchronous latency benchmarking operation.</returns>
    private static async Task BenchmarkLatencyAsync(IRxS7 plc, BenchmarkResult result, BenchmarkConfig config)
    {
    ...
    /// <param name="plc">The PLC instance to benchmark.</param>
    /// <param name="result">The object that receives the benchmark results, including operations per second and any
    /// errors encountered
    /// during the test.</param>
    /// <param name="config">The configuration settings for the benchmark, including the duration of the throughput
    /// test.</param>
    ...
    /// <param name="plc">The PLC instance to benchmark.</param>
    /// <param name="result">The result object that will be updated with reliability statistics and any errors
    /// encountered during the
    /// benchmark.</param>
    /// <param name="config">The configuration settings for the benchmark, including the number of reliability test
    /// iterations to perform.</param>
    /// <returns>A task that represents the asynchronous operation of the reliability benchmark.</returns>
    private static async Task BenchmarkReliabilityAsync(IRxS7 plc, BenchmarkResult result, BenchmarkConfig config)
    {
        var successCount = 0;
    ...
    /// at 100.</remarks>
    /// <param name="result">The benchmark result containing average latency in milliseconds, operations per second, and
    /// reliability rate to
    /// be evaluated.</param>
