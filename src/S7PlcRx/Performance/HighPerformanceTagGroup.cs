...
public class HighPerformanceTagGroup<T> : IDisposable
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="HighPerformanceTagGroup{T}"/> class, associating a set of tag names
    /// with a specified.
    ...
    /// <exception cref="ArgumentException">Thrown if <paramref name="groupName"/> is null, whitespace, or if <paramref
    ...
    public HighPerformanceTagGroup(IRxS7 plc, string groupName, string[] tagNames)
    {
        ...
        foreach (var tagName in tagNames)
        {
            ...
            {
                _ = TagOperations.AddUpdateTagItem(plc, typeof(T), tagName, tagName)
                .SetPolling(false); // Disable individual polling for performance
    ...
    /// <returns>An observable sequence that emits a dictionary containing the latest values for each tag in the group.
    /// Each
    ...
    public IObservable<Dictionary<string, T?>> ObserveGroup()
    {
        foreach (var tagName in _tagNames)
        {
            if (_plc.TagList.ContainsKey(tagName))
            {
                ...
                _ = TagOperations.GetTag(_plc, tagName).SetPolling(true);
            }
        }
    ...
    /// <returns>A dictionary containing the tag names as keys and their associated values of type <typeparamref
    /// name="T"/> as
    ...
    public Task<Dictionary<string, T?>> ReadAllAsync() =>
        AdvancedExtensions.ValueBatchAsync(_plc, default(T), _tagNames);
    ...
    /// <param name="values">A dictionary containing tag names as keys and their corresponding values to be written.
    /// Only entries with tag
    /// names recognized by the PLC will be processed.</param>
    /// <returns>A task that represents the asynchronous write operation.</returns>
    public async Task WriteAllAsync(Dictionary<string, T> values)
    {
        var filteredValues = FilterValues(values);
        await AdvancedExtensions.ValueBatchAsync(_plc, filteredValues);
    }

    /// <summary>Disposes this tag group.</summary>
    public void Dispose()
    {
    ...
    private void DisablePolling()
    {
        foreach (var tagName in _tagNames)
        {
            _ = TagOperations.GetTag(_plc, tagName)
                .SetPolling(false); // Disable individual polling for performance
        }
    }
    ...
    private void RemoveAddedTags()
    {
        foreach (var tagName in _tagsAdded)
        {
            if (_plc.TagList.ContainsKey(tagName))
            {
                TagOperations.RemoveTagItem(_plc, tagName);
            }
        }
