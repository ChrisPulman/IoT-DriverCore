...
{
    /// <summary>Initializes a new instance of the <see cref="AbstractModbusMessage"/> class.</summary>
    ...
    internal AbstractModbusMessage(byte slaveAddress, byte functionCode) =>
        MessageImpl = new(slaveAddress, functionCode);
    ...
    {
        if (frame is null || frame.Length < MinimumFrameSize)
        {
