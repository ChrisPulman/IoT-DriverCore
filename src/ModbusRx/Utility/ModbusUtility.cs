...
public static class ModbusUtility
{
    ...
    /// <summary>Converts four UInt16 values to an IEEE 64-bit floating-point value.</summary>
    ...
    public static double GetDouble(ushort b3, ushort b2, ushort b1, ushort b0)
    {
        ...
        Array.Copy(BitConverter.GetBytes(b0), 0, value, 0, Two);
        Array.Copy(BitConverter.GetBytes(b1), 0, value, Two, Two);
        Array.Copy(BitConverter.GetBytes(b2), 0, value, Four, Two);
        Array.Copy(BitConverter.GetBytes(b3), 0, value, Six, Two);

        return BitConverter.ToDouble(value, 0);
    }

    /// <summary>Converts two UInt16 values to an IEEE 32-bit floating-point value.</summary>
    ...
    public static float GetSingle(ushort highOrderValue, ushort lowOrderValue)
    {
        ...
        Array.Copy(BitConverter.GetBytes(lowOrderValue), 0, value, 0, Two);
        Array.Copy(BitConverter.GetBytes(highOrderValue), 0, value, Two, Two);
    ...
    {
        var result = new List<byte>(numbers.Length * Two);
    ...
    {
        var result = new List<byte>(numbers.Length * Four);
    ...
    /// <summary>Converts network-order bytes to host-order UInt16 values.</summary>
    ...
    public static int NetworkBytesToHostUInt16(ReadOnlySpan<byte> networkBytes, Span<ushort> result)
    {
        ...
        var count = networkBytes.Length / Two;
        ...
        {
            var networkValue = BinaryPrimitives.ReadInt16BigEndian(networkBytes.Slice(i * Two, Two));
    ...
    public static ushort[] NetworkBytesToHostUInt16(byte[] networkBytes)
    {
        ...
        var result = new ushort[networkBytes.Length / Two];

        for (var i = 0; i < result.Length; i++)
        {
            result[i] = (ushort)IPAddress.NetworkToHostOrder(BitConverter.ToInt16(networkBytes, i * Two));
        }
    ...
    public static int HexToBytes(ReadOnlySpan<char> hex, Span<byte> result)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            var hexString = hex.Slice(i * Two, Two).ToString();
            if (!byte.TryParse(hexString, System.Globalization.NumberStyles.HexNumber, null, out var value))
            {
                throw new FormatException($"Invalid hex characters at position {i * Two}.");
            }
    ...
    public static byte[] HexToBytes(string hex)
    {
        ...
        var bytes = new byte[hex.Length / Two];

        for (var i = 0; i < bytes.Length; i++)
        {
            bytes[i] = Convert.ToByte(hex.Substring(i * Two, Two), Sixteen);
        }
    ...
    public static void WriteDouble(double value, Span<ushort> registers, bool swapWords)
    {
        ...
        if (swapWords)
        {
            registers[0] = BitConverter.ToUInt16(bytes, Two);
            registers[1] = BitConverter.ToUInt16(bytes, 0);
            registers[2] = BitConverter.ToUInt16(bytes, Six);
            registers[3] = BitConverter.ToUInt16(bytes, Four);
        }
        else
        {
            registers[0] = BitConverter.ToUInt16(bytes, 0);
            registers[1] = BitConverter.ToUInt16(bytes, Two);
            registers[2] = BitConverter.ToUInt16(bytes, Four);
            registers[3] = BitConverter.ToUInt16(bytes, Six);
        }
    }
    ...
    /// <param name="swapWords">Whether to swap word order.</param>
    /// <exception cref="ArgumentException">Thrown when registers span is too small.</exception>
    public static void WriteSingle(float value, Span<ushort> registers, bool swapWords)
    {
    ...
    /// <exception cref="ArgumentException">Thrown when registers span is too small.</exception>
    public static double ReadDouble(ReadOnlySpan<ushort> registers, bool swapWords)
    {
        if (registers.Length < 4)
        {
            throw new ArgumentException("Registers span must be at least 4 elements.", nameof(registers));
    ...
    /// <exception cref="ArgumentException">Thrown when registers span is too small.</exception>
    public static float ReadSingle(ReadOnlySpan<ushort> registers, bool swapWords)
    {
        if (registers.Length < 2)
        {
            throw new ArgumentException("Registers span must be at least 2 elements.", nameof(registers));
