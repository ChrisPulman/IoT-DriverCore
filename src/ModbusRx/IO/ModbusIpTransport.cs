...
internal sealed class ModbusIpTransport : ModbusTransport
{
    ...
    /// <summary>Gets the transaction identifier lock.</summary>
    private static Lock TransactionIdLock { get; } = new();

    /// <summary>Executes the Read Request Response operation.</summary>
    /// <param name="streamResource">The stream Resource value.</param>
    /// <returns>The result.</returns>
    internal static async Task<byte[]> ReadRequestResponseAsync(IStreamResource streamResource)
    {
        // read header
        var mbapHeader = new byte[6];
        var numBytesRead = 0;

        while (numBytesRead != Six)
        {
            var bytesRead = await streamResource.ReadAsync(mbapHeader, numBytesRead, Six - numBytesRead);
        ...
        var frameLength = (ushort)IPAddress.HostToNetworkOrder(BitConverter.ToInt16(mbapHeader, Four));
    ...
    /// <returns>The result.</returns>
    internal async Task<IModbusMessage> CreateMessageAndInitializeTransactionIdAsync<T>(
        Task<byte[]> fullFrame,
        ...
        where T : IModbusMessage, new()
    {
        ...
        var response = await CreateResponseAsync(messageFrame, responseFactory);
    ...
    internal override Task<byte[]> ReadRequestAsync() =>
        ReadRequestResponseAsync(StreamResource);

    internal override Task<IModbusMessage> ReadResponseAsync<T>(Func<T> responseFactory) =>
        CreateMessageAndInitializeTransactionIdAsync(
            ReadRequestResponseAsync(StreamResource),
            responseFactory);
    ...
    internal override void OnValidateResponse(IModbusMessage request, IModbusMessage response)
    {
        ...
        var msg = $"Response was not of expected transaction ID. Expected {request.TransactionId}, " +
