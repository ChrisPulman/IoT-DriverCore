...
/// <summary>Transport for serial protocols.</summary>
public abstract class ModbusSerialTransport : ModbusTransport
{
    ...
    internal override async Task<IModbusMessage> CreateResponseAsync<T>(
        Task<byte[]> frame,
        Func<T> responseFactory)
    {
        var response = await CreateResponseMessageAsync(frame, responseFactory);

        // compare checksum
        if (CheckFrame && !ChecksumsMatch(response, await frame))
        {
            var msg = $"Checksums failed to match {string.Join(", ", response.ToMessageFrame())} " +
                      $"!= {string.Join(", ", frame)}";
            Debug.WriteLine(msg);
