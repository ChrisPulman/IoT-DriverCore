...
public static class EnronModbusExtensions
{
    ...
    /// <param name="master">The extension receiver.</param>
    ...
    public static Task WriteSingleRegister32Async(
        ModbusMaster master,
        byte slaveAddress,
    ...
    {
        var result = new ushort[registers.Length * Two];
        for (var i = 0; i < registers.Length; i++)
        {
            var bytes = BitConverter.GetBytes(registers[i]);
            result[i * Two] = BitConverter.ToUInt16(bytes, 0);
            result[(i * Two) + 1] = BitConverter.ToUInt16(bytes, Two);
        }
    ...
    {
        var result = new uint[registers.Length / Two];
        for (var i = 0; i < result.Length; i++)
        {
            var registerIndex = i * Two;
