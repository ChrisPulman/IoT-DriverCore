...
public sealed class ModbusServer : IDisposable
{
    ...
    public IDisposable AddTcpClient(string name, string hostAddress, int port, byte slaveAddress)
    {
        ...
        var subscription = Observable.Interval(TimeSpan.FromMilliseconds(OneHundred))
    ...
    /// <param name="slaveAddress">The slave address.</param>
    /// <returns>A disposable subscription.</returns>
    public IDisposable AddUdpClient(string name, string hostAddress, int port, byte slaveAddress)
    {
        ThrowIfNullOrWhiteSpace(name, nameof(name));
    ...
    /// <returns>A disposable subscription.</returns>
    public IDisposable StartTcpServer(int port, byte unitId)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                ModbusDiagnostics.Write($"TCP Server error on port {port}: {ex.Message}");
            }
        });
    ...
    /// <param name="unitId">The unit ID for the slave.</param>
    /// <returns>A disposable subscription.</returns>
    public IDisposable StartUdpServer(int port, byte unitId)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                ModbusDiagnostics.Write($"UDP Server error on port {port}: {ex.Message}");
            }
        });
    ...
    private async Task UpdateDataFromClientAsync(ModbusIpMaster master, byte slaveAddress)
    {
        var dataStore = DataStore;
        if (dataStore is null)
        {
        ...
        try
        {
            ...
            var holdingRegs = await master.ReadHoldingRegistersAsync(slaveAddress, 0, OneHundred);
            var inputRegs = await master.ReadInputRegistersAsync(slaveAddress, 0, OneHundred);
            var coils = await master.ReadCoilsAsync(slaveAddress, 0, OneHundred);
            var inputs = await master.ReadInputsAsync(slaveAddress, 0, OneHundred);
        ...
        {
            ModbusDiagnostics.Write($"Error updating data from client: {ex.Message}");
        }
    }
    ...
    private void StartSimulation()
    {
        ...
        _simulationTimer = Observable.Interval(TimeSpan.FromMilliseconds(FiveHundred))
    ...
    private void UpdateSimulationData()
    {
        ...
        try
        {
            ...
            for (var i = 1; i < Math.Min(OneHundredOne, dataStore.HoldingRegisters.Count); i++)
            {
                dataStore.HoldingRegisters[i] = (ushort)GetRandomInt32(UShortRange);
            }

            for (var i = 1; i < Math.Min(OneHundredOne, dataStore.InputRegisters.Count); i++)
            {
                dataStore.InputRegisters[i] = (ushort)GetRandomInt32(UShortRange);
            }

            for (var i = 1; i < Math.Min(OneHundredOne, dataStore.CoilDiscretes.Count); i++)
            {
                dataStore.CoilDiscretes[i] = GetRandomBoolean();
            }

            for (var i = 1; i < Math.Min(OneHundredOne, dataStore.InputDiscretes.Count); i++)
            {
