...
public class ModbusSerialTransportFixture
{
    ...
    {
        using var transport = new ModbusAsciiTransport(StreamResource) { CheckFrame = true };
        var frame = Task.FromResult<byte[]>([ Num.Value19, Modbus.ReadCoils, 0, 0, 0, Num.Value2, Num.Value115]);

        await Assert.ThrowsAsync<IOException>(() => transport.CreateResponseAsync(
            frame,
    ...
    /// <returns>A <see cref="Task"/> representing the asynchronous unit test.</returns>
    [TUnit.Core.Test]
    public async Task CreateResponseErroneousLrcDoNotCheckFrameAsync()
    {
        using var transport = new ModbusAsciiTransport(StreamResource) { CheckFrame = false };
        var frame = Task.FromResult<byte[]>([ Num.Value19, Modbus.ReadCoils, 0, 0, 0, Num.Value2, Num.Value115]);

        await transport.CreateResponseAsync(frame, static () => new ReadCoilsInputsResponse());
    }

    /// <summary>
    /// When using the serial RTU protocol, the beginning of the message can get mangled,
    /// leading to an unsupported message type.
    ...
    public void UnicastMessage_PurgeReceiveBuffer()
    {
        ...
        using var transport = new ModbusRtuTransport(serialResource);
        ...
        var actualResponse = transport.UnicastMessage(request, static () => new ReadCoilsInputsResponse());
