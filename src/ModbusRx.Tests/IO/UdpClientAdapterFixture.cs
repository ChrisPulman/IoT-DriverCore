...
public class UdpClientAdapterFixture
{
    ...
    {
        using var adapter = new UdpClientAdapter(new UdpClientRx());

        // buffer
        await Assert.ThrowsAsync<ArgumentNullException>(() => adapter.ReadAsync(null!, 1, 1));

        // offset
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() => adapter.ReadAsync(new byte[2], -1, Num.Value2));
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(
            () => adapter.ReadAsync(new byte[2], Num.Value3, Num.Value3));
    ...
    public void Write_ArgumentValidation()
    {
        ...
        // offset
        _ = Assert.Throws<ArgumentOutOfRangeException>(() => adapter.Write(new byte[2], -1, Num.Value2));
        _ = Assert.Throws<ArgumentOutOfRangeException>(() => adapter.Write(new byte[2], Num.Value3, Num.Value3));
    }
}
