...
public class ModbusRtuTransportFixture
{
    ...
    public void BuildMessageFrame()
    {
        ...
        var request = new ReadCoilsInputsRequest(Modbus.ReadCoils, Num.Value17, Num.Value19, Num.Value37);
    ...
    public void ResponseBytesToReadCoils()
    {
        ...
        Assert.Equal(Num.Value6, ModbusRtuTransport.ResponseBytesToRead(frameStart));
    }
    ...
    public void ResponseBytesToReadWriteCoilsResponse()
    {
        ...
        Assert.Equal(Num.Value4, ModbusRtuTransport.ResponseBytesToRead(frameStart));
    }
    ...
    public void RequestBytesToReadWriteCoilsRequest()
    {
        ...
        Assert.Equal(Num.Value4, ModbusRtuTransport.RequestBytesToRead(frameStart));
    }
    ...
    public void RequestBytesToReadWriteMultipleHoldingRegisters()
    {
        ...
        Assert.Equal(Num.Value6, ModbusRtuTransport.RequestBytesToRead(frameStart));
    }
    ...
    {
        using var transport = new ModbusRtuTransport(StreamResource);
        var message = new ReadCoilsInputsRequest(Modbus.ReadCoils, Num.Value17, Num.Value19, Num.Value38);
        byte[] frame = { 17, Modbus.ReadCoils, 0, 19, 0, 37, 14, 132 };
    ...
    [TUnit.Core.Test]
    public async Task ReadResponseAsync()
    {
        var mock = CreateReadStreamResource([1, 1, 1, 0, Num.Value81, Num.Value136]);
        using var transport = new ModbusRtuTransport(mock.Object);

        var response = await transport.ReadResponseAsync(static () => new ReadCoilsInputsResponse());
    ...
    /// <returns>A <see cref="Task"/> representing the asynchronous unit test.</returns>
    [TUnit.Core.Test]
    public async Task ReadResponseSlaveExceptionAsync()
    {
    ...
    public async Task ReadResponseSlaveExceptionWithErroneousLrcAsync()
    {
        ...
        await Assert.ThrowsAsync<IOException>(() => transport.ReadResponseAsync(
    ...
    {
        var mock = CreateReadStreamResource([1, 1, 1, 0, 1, 0, 0, Num.Value5]);
        using var transport = new ModbusRtuTransport(mock.Object);

        Assert.Equal([ 1, 1, 1, 0, 1, 0, 0, Num.Value5], await transport.ReadRequestAsync());
    ...
    public void Read()
    {
        ...
        using var transport = new ModbusRtuTransport(mock.Object);
        Assert.Equal([ Num.Value2, Num.Value2, Num.Value2, Num.Value3, Num.Value3], transport.Read(Num.Value5));
